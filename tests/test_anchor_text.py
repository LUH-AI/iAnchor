...
def test_text_greedy_search():
    explainer = Anchor(Tasktype.TEXT)
    ...
    assert anchor.feature_mask == [3]
    assert np.isclose(anchor.coverage, -1)
...
def test_text_beam_search():
    ...
    assert anchor.feature_mask == [4, 3, 0]
    assert np.isclose(anchor.coverage, 0.0)
